...
const TRIP_DATA = {
  ...
  goBag: {
    always: [
      "4-6 diapers + wipes", "1 change of clothes per kid", "2x power banks + cables",
  ...
  days: [
    ...
    },
    {
      ...
      timeline: [
        { time: "08:30", end: "10:15", activity: "Dusit Central Park breakfast", type: "food", dur: 105, location: "Dusit Central Park", lat: 13.7255, lng: 100.5290, floor: "Parkside Market, LG Floor", details: "Basecamp: corner booth, pram as 4th chair. Vendors: Pranom chicken rice + Cheese Roti." },
        ...
        { time: "13:00", end: "13:30", activity: "Siam Paragon food", type: "food", dur: 30, location: "Siam Paragon", details: "Thongyoy Café GF (Thai tea). After You backup." },
        { time: "13:30", end: "16:00", activity: "MBK Center food crawl", type: "food", dur: 150, location: "MBK Center", lat: 13.7441, lng: 100.5300, floor: "6F & 7F food courts", details: "Skywalk from Siam. Basecamp: pillar tables. Mo-Mo 6F, Takoyaki hunt 5-6F, Guay Jab, Pad Thai." },
      ...
      food: [
        ...
        { name: "Mo-Mo-Paradise Shabu", where: "MBK 6F", thai: null, order: "Buffet, 259-450 THB/person. 1hr 40min limit.", kid: true },
        { name: "Takoyaki", where: "MBK 5-6F", thai: null, order: "Walk entire food court systematically", kid: true },
        { name: "Guay Jab Nam Khon", where: "MBK 6F", thai: null, order: "Point and order", kid: true, visual: "Dark broth, rolled noodles, crispy pork." },
    ...
    },
    {
      ...
      timeline: [
        ...
        { time: "08:30", end: "10:30", activity: "Dusit Central Park breakfast", type: "food", dur: 120, location: "Dusit Central Park", lat: 13.7255, lng: 100.5290, details: "Vendors: Nakhon Pathom Tom Yum Noodle (Michelin — order Nam Sai for kids) + Thong Smith Boat Noodles." },
    ...
    },
    {
      ...
      timeline: [
        { time: "08:30", end: "10:15", activity: "Dusit breakfast", type: "food", dur: 105, location: "Dusit Central Park", lat: 13.7255, lng: 100.5290, details: "Vendors: Shangra Law House shrimp noodles + New Camp beef skewers." },
    ...
    },
    {
      ...
      timeline: [
        { time: "08:30", end: "10:15", activity: "Dusit breakfast", type: "food", dur: 105, location: "Dusit Central Park", lat: 13.7255, lng: 100.5290, details: "Vendors: Thai Crispy Crepe + Isan Som Tam & Gai Yang (kids: chicken + sticky rice)." },
    ...
    },
    {
      ...
      timeline: [
        { time: "08:30", end: "10:15", activity: "Dusit breakfast", type: "food", dur: 105, location: "Dusit Central Park", lat: 13.7255, lng: 100.5290, details: "Vendors: Adults treat — Sakon Nakhon Som Tam (NOT for kids) + Bottomless Coffee 5F. Kids: repeat Pranom." },
    ...
    },
    {
      ...
      timeline: [
        { time: "08:30", end: "10:15", activity: "Dusit breakfast", type: "food", dur: 105, location: "Dusit Central Park", lat: 13.7255, lng: 100.5290, details: "Wildcard — repeat favourites or try new." },
        ...
        { time: "17:30", end: "18:15", activity: "Taxi home", type: "transit", dur: 45, details: "With shopping bags." },
      ],
      food: [
    ...
    },
    {
      ...
      timeline: [
        { time: "08:30", end: "10:15", activity: "Dusit breakfast", type: "food", dur: 105, location: "Dusit Central Park", lat: 13.7255, lng: 100.5290, details: "Any remaining untried stall." },
    ...
    },
    {
      ...
      timeline: [
        { time: "08:30", end: "10:15", activity: "Dusit — revisit #1 favourite", type: "food", dur: 105, location: "Dusit Central Park", lat: 13.7255, lng: 100.5290, details: "Make it memorable." },
    ...
    },
    {
      ...
      timeline: [
        { time: "08:30", end: "10:15", activity: "Dusit — FINAL breakfast", type: "food", dur: 105, location: "Dusit Central Park", lat: 13.7255, lng: 100.5290, details: "Last bowl. Savour it." },
...
// ============ LOCATION MAPPING ============
// Maps food items to normalized location keys for carry-forward
const LOCATION_KEYS = {
...
// Build a map of which location keys each day visits
...
// Get missed food items from previous days that match today's locations
function getMissedFoodForDay(dayIndex, days, checkedFood) {
  ...
  for (let i = 0; i < dayIndex; i++) {
    ...
    pastDay.food.forEach(item => {
      ...
      if (isChecked) return; // already tried
      
      const itemLocKey = getLocationKey(item.where);
      if (itemLocKey && todayKeys.has(itemLocKey)) {
        // Check it's not already a native item today (same name)
        const alreadyToday = todayData.food.some(f => f.name === item.name);
        // Check it wasn't already carried forward and checked on a day between
...
function getGoogleMapsLink(lat, lng, label) {
...
function TimelineCard({ item, isActive, onActivate, activeTimers, onToggleTimer }) {
  const [expanded, setExpanded] = useState(false);
  const [showThai, setShowThai] = useState(null);
  ...
  return (
    <div
      style={{
        background: isOvertime ? "#fef2f2" : "#fff",
        borderRadius: 12,
        marginBottom: 8,
        border: `1.5px solid ${isOvertime ? "#fca5a5" : isActive ? typeColors[item.type] : "#e5e7eb"}`,
        overflow: "hidden",
        transition: "all 0.2s",
        boxShadow: isActive ? `0 2px 8px ${typeColors[item.type]}33` : "none"
      }}
    >
      <div
...
export default function BangkokApp() {
  ...
  // Compute missed food items that carry forward to today
  ...
  const dayBudgets = TRIP_DATA.days.map(d => {
    const avg = (s) => { if (!s || s === "Variable") return 0; const parts = s.split("-").map(n => parseInt(n.replace(/[^\d]/g, ""))); return parts.length === 2 ? (parts[0] + parts[1]) / 2 : parts[0] || 0; };
    ...
    <div style={{ maxWidth: 430, margin: "0 auto", background: "#f8fafc", minHeight: "100vh", fontFamily: "'Nunito', -apple-system, sans-serif", position: "relative", paddingBottom: 70 }}>
        ...
        {activeTab === "timeline" && (
          <div>
            ...
            {day.timeline.map((item, i) => (
              <TimelineCard key={`${day.date}-${i}`} item={item} isActive={false} onActivate={() => {}} activeTimers={activeTimers} onToggleTimer={toggleTimer} />
            ))}
          </div>
        )}
              ...
              <>
                ...
                {missedFood.length > 0 && (
                  <>
                    ...
                    {missedFood.map((item) => {
                      // Find original date for this item
      ...
      {showPanel && (
        <div style={{ position: "fixed", bottom: 56, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 430, background: "#fff", borderTop: "1px solid #e5e7eb", borderRadius: "16px 16px 0 0", boxShadow: "0 -4px 20px rgba(0,0,0,0.1)", maxHeight: "55vh", overflow: "auto", zIndex: 49, padding: 16 }}>
          ...
          {showPanel === "transport" && (
            <div>
              {day.timeline.filter(t => t.lat && t.lng).map((t, i) => (
                <a
                  key={i}
                  href={getGoogleMapsLink(t.lat, t.lng)}
                  target="_blank"
